...
3. Write the lecture note in `10-Programme/Modules/<CODE> - <Title>/` from the `Lecture note` template, wikilinking every concept met. Lab notes are `Tutorial`, paper notes `Reference` (always with a `resource`), concept notes `Explanation`, each from its template.
4. Read the vault through `01-Dashboard/Vault.base` - Inbox, Lectures grouped by `module`, Papers, Concepts, Daily - live tables over the notes' properties; `Home` embeds the Inbox view.

## The weekly sweep

Empty the inbox: every note gets a `type` and a `title` and moves to its lifecycle folder, or is deleted. Then ask which concept note in `30-Knowledge/Concepts` has stopped changing, is linked from two or more notes (the Backlinks pane shows this), and would be annoying to find wrong in six months. That note graduates: it gets a `resource`, a `description` and a `status`; `lokf-librarian` derives the record into `concepts/` here; `lokf-curator`, or the LOKF Curator plugin in this vault, confirms it.
