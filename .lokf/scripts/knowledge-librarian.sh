...
# and MSc-AI/knowledge_bundle/ when that is the real folder and .lokf/knowledge the
...
outside_bundle() {
  git status --porcelain -- '.' \
    ':(exclude).lokf/knowledge' ':(exclude)MSc-AI/knowledge_bundle' ':(exclude).lokf/feedback.md' \
    | cut -c4- | sort -u
}
