...
`msc-ai-galway-2026` is a personal Obsidian vault: example notes, a validated LOKF knowledge bundle, and the tooling that keeps both in shape. It ships no plugin, no server, and no runtime code of its own - opening the vault in Obsidian runs nothing beyond whatever community plugins you already have installed there (see the README's "Plugins" and "Upstream" sections for which ones).
